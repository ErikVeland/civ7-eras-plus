// Insert the "Custom (per-age)" preset into every language's Pacing-presets
// list in the Workshop description. Run once, delete. Validate BBCode after.
import fs from "fs";
// header text -> [preset name, one-line description]
const T = {
  ...
  "Malay": ["Tersuai (setiap era)", "Tetapkan panjang era Purba, Penerokaan dan Moden secara berasingan."],
  ...
  "Greek": ["Προσαρμοσμένο (ανά εποχή)", "Ορίστε ξεχωριστά τη διάρκεια της Αρχαιότητας, της Εξερεύνησης και της Σύγχρονης εποχής."],
...
const path = "workshop-description-translations-bbcode.md";
...
const out = parts.map((chunk) => {
  ...
  const presetIdx = chunk.indexOf("[h2]Pacing presets[/h2]");
  if (presetIdx < 0) return chunk;
  const listEnd = chunk.indexOf("[/list]", presetIdx);
  if (listEnd < 0) return chunk;
  const line = `[*][b]${entry[0]}:[/b] ${entry[1]}\n`;
  inserted++;
  return chunk.slice(0, listEnd) + line + chunk.slice(listEnd);
});
fs.writeFileSync(path, out.join("\n"));
console.log(`inserted Custom preset into ${inserted} sections`);
